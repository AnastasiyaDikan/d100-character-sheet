...
function StartScreen({ autosave, onNew, onLoad, onRestore, onDeleteAutosave }: {
  ...
  return <main className="start-screen">
    ...
    <section className="start-card" aria-labelledby="start-title">
      ...
      <p className="version">Character Sheet v0.3.5</p>
    </section>
  </main>;
}
...
function BackPage({ character, onChange }: { character: Character; onChange: (character: Character) => void }) {
  const move = movement(character), carry = carrying(character), fatigue = fatigueThreshold(character), spent = spentExperience(character);
  const update = <K extends keyof Character>(key: K, value: Character[K]) => onChange({ ...character, [key]: value });
  ...
  return <div className="sheet-page back-page"><div className="back-grid"><div className="equipment-column">
    ...
    <section className="panel derived-panel"><div><h3>Раны</h3><CalculatedNumber label="Всего (автоматически)" value={character.woundsTotal} /><NumberField label="Сейчас" value={character.woundsCurrent} onChange={(value) => update("woundsCurrent", value)} /></div><div><h3>Движение</h3><dl><div><dt>Свободное</dt><dd>{move.free}</dd></div><div><dt>Полудействие</dt><dd>{move.halfAction}</dd></div><div><dt>Натиск</dt><dd>{move.charge}</dd></div><div><dt>Бег</dt><dd>{move.run}</dd></div></dl></div><div><h3>Носить / поднимать</h3><dl><div><dt>Носить</dt><dd>{carry.carry} кг</dd></div><div><dt>Поднимать</dt><dd>{carry.lift} кг</dd></div><div><dt>Толкать</dt><dd>{carry.push} кг</dd></div></dl></div><div><h3>Усталость</h3><div className="threshold-display"><span>Порог</span><strong>{fatigue}</strong></div><NumberField label="Сейчас" value={character.fatigueCurrent} onChange={(value) => update("fatigueCurrent", value)} /></div><div><h3>Опыт</h3><NumberField label="Получено" value={character.experienceEarned} onChange={(value) => update("experienceEarned", value)} /><dl><div><dt>Потрачено</dt><dd>{spent}</dd></div><div><dt>Доступно</dt><dd className={character.experienceEarned - spent < 0 ? "negative" : ""}>{character.experienceEarned - spent}</dd></div></dl></div><div><h3>Естественная броня</h3><NumberField label="КД" value={character.naturalArmor} onChange={(value) => update("naturalArmor", value)} /><p className="formula-note"><Shield /> учитывается во всех зонах</p></div></section>
