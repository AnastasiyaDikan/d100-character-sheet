...
import { bonus, effectiveBonus, fatiguedEffectiveBonus, supernaturalBonus, supernaturalMultiplier, zoneDefense } from "@/lib/character/calculations";
...
export default function BodyMap({ character }: { character: Character }) {
  ...
  const enduranceAdditive = supernaturalBonus(character, "endurance");
  ...
  return <section className="panel hit-map" data-tutorial="hit-map">
    <div className="section-heading"><h2>Карта попаданий</h2><span>{character.gender === "female" ? "Женщина" : "Мужчина"} · БВ {currentEnduranceBonus}{currentEnduranceBonus !== normalEnduranceBonus ? ` (до усталости ${normalEnduranceBonus})` : enduranceMultiplier > 1 && baseEnduranceBonus * enduranceMultiplier >= baseEnduranceBonus + enduranceAdditive ? ` (${baseEnduranceBonus}×${enduranceMultiplier})` : enduranceAdditive > 0 ? ` (${baseEnduranceBonus}+${enduranceAdditive})` : ""}</span></div>
